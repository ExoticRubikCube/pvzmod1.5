...
public class EntityUtil {
    ...
    public static Entity createWithNBT(Level world, EntityType<?> entityType, CompoundTag nbt, BlockPos pos) {
        ...
        compound.putString("id", ForgeRegistries.ENTITY_TYPES.getKey(entityType).toString());
    ...
    public static boolean canEntityBeRemoved(Entity entity) {
        ...
        if (!ForgeRegistries.ENTITY_TYPES.getKey(entity.getType()).getNamespace().equals(PVZMod.MOD_ID)
    ...
    public static float getCurrentDefenceHealth(LivingEntity entity) {
        ...
        if (entity.getAttribute(PVZAttributes.INNER_DEFENCE_HP.get()) != null) {
            health += (float) entity.getAttribute(PVZAttributes.INNER_DEFENCE_HP.get()).getValue();
        }
        if (entity.getAttribute(PVZAttributes.OUTER_DEFENCE_HP.get()) != null) {
            health += (float) entity.getAttribute(PVZAttributes.OUTER_DEFENCE_HP.get()).getValue();
        }
        return health;
    }
    ...
    public static boolean canSeeEntity(Entity entity, Entity target) {
        ...
        return entity.level.clip(ray1).getType() != HitResult.Type.BLOCK || entity.level.clip(ray2).getType() != HitResult.Type.BLOCK;
    }
    ...
    public static void onEntitySpawn(LevelAccessor world, Entity entity, BlockPos pos) {
        ...
        if (entity instanceof Mob mob) {
            mob.finalizeSpawn((ServerLevel) world, world.getCurrentDifficultyAt(entity.blockPosition()), MobSpawnType.SPAWNER, null, null);
        }
    ...
    public static boolean checkCanEntityBeTarget(Entity attacker, Entity target) {
        ...
        if (attacker instanceof LivingEntity living) {//target the entity who attack it before.
            if (living.getLastHurtMob() != null && target.is(living.getLastHurtMob()) && checkCanEntityBeAttack(attacker, target)) {
                return true;
            }
        }
    ...
    public static boolean hasNearBy(Level world, BlockPos pos, double r, Predicate<Entity> pre) {
        return world.getEntitiesOfClass(Entity.class, BlockUtil.getAABB(pos, r, r)).stream().anyMatch(pre::test);
    }
    ...
    public static EntityHitResult rayTraceEntities(Level world, Entity entity, Vec3 lookVec, double distance, Predicate<Entity> predicate) {
        ...
        HitResult result = world.clip(ray);
    ...
    public static boolean isOpEntity(Entity entity) {
        if (entity instanceof Player player && !PlayerUtil.isPlayerSurvival(player)) {
            return true;
        }
