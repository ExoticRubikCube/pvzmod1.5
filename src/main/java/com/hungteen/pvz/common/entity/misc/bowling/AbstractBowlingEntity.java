...
this.discard();
	...
	private void tickRayTrace() {
		...
		Vec3 end = start.add(new Vec3(dx * rayLen, 0, dz * rayLen));
		...
		if(result != null && result.getType() != HitResult.Type.MISS) { // Projectile event skipped for 1.19 migration
...
//			this.rotationYaw = (float) (Mth.atan2(x, z) * (double) (180F / (float) Math.PI));
//			this.rotationPitch = (float) (Mth.atan2(y, (double) f) * (double) (180F / (float) Math.PI));
