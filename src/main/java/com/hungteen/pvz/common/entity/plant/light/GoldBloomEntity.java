	...
	protected void normalPlantTick() {
		...
		if(! this.level.isClientSide()) {
			...
			if(time == 16) {
				...
				SunEntity.dropSunRandomly(level, this.blockPosition().above(), 50, 0.3F);
				SunEntity.dropSunRandomly(level, this.blockPosition().above(), 25, 0.3F);
				SunEntity.dropSunRandomly(level, this.blockPosition().above(), 5, 0.4F);
				SunEntity.dropSunRandomly(level, this.blockPosition().above(), 5, 0.4F);
			} else if(time == 29) {
				EntityUtil.playSound(this, SoundRegister.GOLD_BLOOM_PRODUCE.get());
				SunEntity.dropSunRandomly(level, this.blockPosition().above(), 50, 0.3F);
				...
				SunEntity.dropSunRandomly(level, this.blockPosition().above(), 50, 0.3F);
				SunEntity.dropSunRandomly(level, this.blockPosition().above(), 25, 0.3F);
				SunEntity.dropSunRandomly(level, this.blockPosition().above(), 25, 0.4F);
				SunEntity.dropSunRandomly(level, this.blockPosition().above(), 15, 0.4F);
				SunEntity.dropSunRandomly(level, this.blockPosition().above(), 15, 0.4F);
				SunEntity.dropSunRandomly(level, this.blockPosition().above(), 10, 0.4F);
			}
		}
	}
