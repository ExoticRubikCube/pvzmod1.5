...
public class SunRender extends EntityRenderer<SunEntity> {
	...
	@Override
	public void render(SunEntity sun, float p_114600_, float partialTicks, PoseStack stack, MultiBufferSource bufferSource, int p_114604_) {
		stack.pushPose();
		...
		int green;
		int alpha;
		red = (int) (sun.ColorBase.x + Mth.sin(tick) * sun.ColorChange.x);
		green = (int) (sun.ColorBase.y + Mth.sin(tick) * sun.ColorChange.y);
		blue = (int) (sun.ColorBase.z + Mth.sin(tick) * sun.ColorChange.z);
		...
		vertex(vertexconsumer, matrix4f, matrix3f, -1F, -1.0F, red, green, blue, x + 12.01F * Mth.cos(tick) / 64, y + 12.01F * Mth.sin(tick) / 64, p_114604_, alpha);
		vertex(vertexconsumer, matrix4f, matrix3f, 1F, -1.0F, red, green, blue, x + 12.01F * Mth.cos(tick + 1.57F) / 64, y + 12.01F * Mth.sin(tick + 1.57F) / 64, p_114604_, alpha);
		vertex(vertexconsumer, matrix4f, matrix3f, 1F, 1.0F, red, green, blue, x - 12.01F * Mth.cos(tick) / 64, y - 12.01F * Mth.sin(tick) / 64, p_114604_, alpha);
		vertex(vertexconsumer, matrix4f, matrix3f, -1F, 1.0F, red, green, blue, x - 12.01F * Mth.cos(tick + 1.57F) / 64, y - 12.01F * Mth.sin(tick + 1.57F) / 64, p_114604_, alpha);
		stack.popPose();
